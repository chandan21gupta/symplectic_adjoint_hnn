...
def generate_chain_datasets(dataset_index, T_train, T_test, n_samples_train, n_samples_test, chain_length, dt=0.1, coarsening_factor=10):
    ...
    np.save('./train_data_chain_' + str(dataset_index), train_data_chain)
    ...
    np.save('./test_data_chain_' + str(dataset_index), test_data_chain)
...
def add_noise_chain(dataset_index, coarsening_factor, noise_level):
    train_data = np.load('./train_data_chain_' + str(dataset_index) + '.npy')
    noise = torch.randn(train_data.shape).numpy() * noise_level
    train_data += noise
    np.save('./train_data_chain_' + str(dataset_index) + '_n' + str(noise_level), train_data)
    test_data = np.load('./test_data_chain_' + str(dataset_index) + '.npy')
    noise = torch.randn(test_data.shape).numpy() * noise_level
    test_data += noise
    np.save('./test_data_chain_' + str(dataset_index) + '_n' + str(noise_level), test_data)
